"""
Gemini 2.5 Pro Browse Adapter for VERA
...
class Gemini25ProBrowseAdapter:
    """Gemini 2.5 Pro adapter with google_search tool for browsecomp."""
    ...
    async def process_episodes_batch(self, episodes: List[Dict[str, Any]], output_dir: str,
        ...
        print(f"[Gemini 2.5 Pro Browse] Batch processing {len(episodes)} episodes (max {max_concurrent} concurrent)")
        ...
        batch_file = output_path / f"gemini_25_pro_browse_batch_{int(time.time())}.json"
        with open(batch_file, 'w') as f:
            json.dump(batch, f, indent=2)

        print(f"[Gemini 2.5 Pro Browse] Batch completed: {batch['successful']}/{len(episodes)} successful")
        return batch
    ...
    def process_episode(self, episode_data: Dict[str, Any], output_dir: str) -> Dict[str, Any]:
        ...
        print(f"[Gemini 2.5 Pro Browse] Processing episode {episode_id}")
        ...
        results_file = output_path / f"gemini_25_pro_browse_{episode_id}.json"
        with open(results_file, 'w') as f:
            json.dump(result, f, indent=2)

        print(f"[Gemini 2.5 Pro Browse] Episode {episode_id} completed in {session_duration:.2f}s")
        return result
...
def test_gemini_browse():
    """Test the Gemini 2.5 Pro Browse adapter"""
    ...
    # Test single episode
    adapter = Gemini25ProBrowseAdapter(api_key)
